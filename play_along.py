...
class PianoVisualizer:
    ...
    def generate_piano_sounds(self):
        """Load piano sound samples from the 'samples' directory."""
        print("Loading sounds from samples directory...")
        ...
        for note in range(self.first_note, self.first_note + self.total_keys):
            note_name = self.get_note_name(note)  # Get e.g., "C#4"
            # Construct filename, handling '#' for sharps
            filename = f"piano_{note_name.replace('#', '#')}.wav"
            ...
            if os.path.exists(file_path):
                ...
                # print(f"Warning: Sample file not found: {file_path}") # Optional: Reduce console spam
                missing_count += 1

        print(f"Sounds loaded: {loaded_count}, Missing: {missing_count}")

    def play_note(self, note, velocity=100):
        """Play the sound for a given note."""
        if note in self.sounds:
            sound = self.sounds[note]
            # Adjust volume based on self.volume and MIDI velocity
            adjusted_volume = self.volume * (velocity / 127.0)
            ...
            if channel:
                ...
                print("Warning: No free channels to play sound.")
